    ...
    class Server
    {
        ...
        public void start()
        {
            ...
            try
            {
                ...
                while (client.Connected)
                {
                    ...
                    if (this.response.StartsWith("USER"))
                    {
                    ...
                    switch (cmd)
                    {
                        case USER:
                            ...
                            response = userFromClient(argv[0]);
                            if (!response.StartsWith("PASS"))
                            {
                                writeMessage("530 Enter Password");
                            }
                            passwordFromClient(argv[0]);
                            break;
        ...
        private void setPassiveMode()
        {
            try
            {
                ...
                var message = "227 Enter Passive Mode (" + localIP.Replace('.', ',') + "," + dataPort + ")";
        ...
        private void writeMessage(string message)
        {
            try
            {
                outputStreamClient.Write(message);
            }
            catch (Exception e)
            {
        ...
        private string userFromClient(string username)
        {
        ...
        private void passwordFromClient(string passwordInput)
        {
            password = passwordInput;
            if (passwordInput.Length >= 0)
            {
                writeMessage("Password Recieved.");
        ...
        private string setTransferType(string transferType)
        {
            var response = "";
            switch (transferType)
            {
